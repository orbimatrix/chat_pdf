...
def reduce_image_quality(pdf: bytes, quality: int, password: str) -> bytes:
    ...
    for page in writer.pages:
        for img in page.images:
            # Check if img.image is already a Pillow image
            if isinstance(img.image, Image.Image):
                pil_image = img.image
            else:
                pil_image = Image.open(BytesIO(img.image))

            # Convert RGBA to RGB if needed
            if pil_image.mode == "RGBA":
                pil_image = pil_image.convert("RGB")

            # Save the image with reduced quality to a byte stream
            img_stream = BytesIO()
            pil_image.save(img_stream, format="JPEG", quality=quality)
            img_stream.seek(0)

            # Reopen the image from the byte stream to ensure it's a Pillow Image
            reduced_image = Image.open(img_stream)

            # Replace the image in the PDF
            img.replace(reduced_image)
